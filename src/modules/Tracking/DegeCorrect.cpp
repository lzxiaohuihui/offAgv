...
bool Tracking::degenerateCorrect(Frame* pFrame, float degeSpan, float degeTheta){
    ...
    float refTheta = degeTheta - pFrame->pose.theta;
    //delte the pixel which direct is far from refTheta
...
void Tracking::getCandidatePose(Frame* pFrame, float degeSpan, float degeTheta){
    ...
    float correctStep = 2.0f * pMap->gaussSize[searchLevel];
    ...
    for(float deltaTrans = -degeSpan; deltaTrans < degeSpan; deltaTrans += correctStep){
        ...
        cout<<"correct error count: "<<error<<" / "<<corPixelCount<<endl;// corPixelCount is number of pixel which gradtheta near reftheta

        //find the local minimum
        if(error < minErr){

            if(error*4.0f < (float)corPixelCount){//????? error*4????
