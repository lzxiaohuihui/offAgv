...
bool Tracking::getLocalMap(const cv::Mat &temp_frame, Eigen::Vector3f params, vector<cv::Point2i> &local_map, vector<cv::Point2i> &icp_frame)
{
    ...
    cv::Mat temp_map = pMap->rawData;

    if (temp_map.channels() == 3)
        cv::cvtColor(temp_map, temp_map, CV_RGB2GRAY);

    if (temp_frame.channels() == 3)
        cv::cvtColor(temp_frame, temp_frame, CV_RGB2GRAY);

    cv::threshold(temp_map, temp_map, 30, 255, CV_THRESH_BINARY);
    cv::threshold(temp_frame, temp_frame, 30, 255, CV_THRESH_BINARY);
    int edge_thresh = 50;

    // cv::Canny(temp_frame, temp_frame, edge_thresh, edge_thresh * 3);
    cv::Canny(temp_map, temp_map, edge_thresh, edge_thresh * 3);

    // cout << "type: " << temp_frame.type() << endl;
    // cout << "type: " << temp_map.type() << endl;
    ...
    for (int i = 0; i < temp_frame.rows; ++i)
    {
        for (int j = 0; j < temp_frame.cols; ++j)
        {
            ...
            // 地图点 Rx + t
            float map_x = (Rp_x + temp_tx);
            float map_y = (Rp_y + temp_ty);
    ...
    // cv::imshow("show_local_map", show_local_map);
    // cv::waitKey(10);

    // cout << "localmap Size: " << local_map.size() << "--continue: " << (count > MIN_NOEMPTY_PIXEL) << endl;
...
bool Tracking::findDataAssociation(cv::Mat &ICP_ref_pts, cv::Mat &ICP_new_pts, vector<cv::Point> &nearest_pts, int knn, float &err)
{
    ...
    for (int i = 0; i < ICP_new_pts.rows; i++)
    {
        ...
        for (int index : near_point_index)
        {
            ...
            err = sqrtf((ICP_new_pts.at<float>(i, 0) - near_x) * (ICP_new_pts.at<float>(i, 0) - near_x) +
                        (ICP_new_pts.at<float>(i, 1) - near_y) * (ICP_new_pts.at<float>(i, 1) - near_y));
        }
        nearest_pts.push_back(cv::Point(near_x / knn, near_y / knn));
    }
...
bool Tracking::icpTrackingWithG_N(vector<cv::Point> &ref_pt_vec, vector<cv::Point> &new_pt_vec)
{
    ...
    cout << "++++++++++++++++++frame no pixels num: " << ICP_new_pts.rows << endl;
    My_Kdtree.build(ICP_ref_pts, cv::flann::KDTreeIndexParams(1), cvflann::FLANN_DIST_EUCLIDEAN);
    ...
    for (int iter_num = 0; iter_num < ITER_MAX; ++iter_num)
    {
        ...
        findDataAssociation(ICP_ref_pts, ICP_new_pts, nearest_pts, 1, err);
        ...
        for (int i = 0; i < ICP_new_pts.rows; ++i)
        {
            int u = ICP_new_pts.at<float>(i, 0);
            int v = ICP_new_pts.at<float>(i, 1);
        ...
        float temp_total_tx = total_tx;
        float temp_total_ty = total_ty;

        total_theta += update[2];
        total_tx = cos_delta * temp_total_tx - sin_delta * temp_total_ty + update[0];
        total_ty = sin_delta * temp_total_tx + cos_delta * temp_total_ty + update[1];
    ...
    float temp_tx = curParams(0);
    float temp_ty = curParams(1);

    curParams(0) = cos_total_theta * temp_tx - sin_total_theta * temp_ty + total_tx;
    curParams(1) = sin_total_theta * temp_tx + cos_total_theta * temp_ty + total_ty;
